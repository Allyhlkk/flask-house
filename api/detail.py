...
def scatter_data(block):
    ...
    #代码实际执行的sql语句
    # SELECT AVG(price/area) FROM house_info WHERE block = '朝阳-工体' GROUP BY pulish_time ORDER BY pulish_time;
    ...
    for i in range(1, 30):
        ...
        #将1天的日期格式化为字符串的形式并添加奥date_list列表中
        #y = 年 m =月 d =日
...
def broke_line_date(block):
    ...
    date_list = []
    for i in range(1, 14):
        #将时问戳(timestamp)转换成具体的日期
        latest_release = datetime.fromtimestamp(int(time_stamp[0][0]))
        #获取最新房源发布时间的i天
        day = latest_release + timedelta(days=-i)
        #将1天的日期格式化为字符串的形式并添加到data_list列表中
        #y = 年 m = 月 d= 日
        date_list.append(day.strftime("%m-%d"))
    #将日期列表反转
    date_list.reverse()
